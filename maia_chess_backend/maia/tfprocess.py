...
#     def get_config(self):
#         config = {}#super(ApplySqueezeExcitation, self).get_config()
#         config.update({'reshape_size':self.reshape_size})
#         return config
...
#     def get_config(self):
#         config = {}#super(ApplyPolicyMap, self).get_config()
# #         print(self.fc1)
#         config.update({'fc1':self.fc1.numpy()})
#         return config
...
class TFProcess:
    def __init__(self, cfg, name, collection_name):
        ...
        # Input shape
        if 'input_size' in self.cfg['model']:
            self.INPUT_SIZE = self.cfg['model']['input_size']
        else:
            self.INPUT_SIZE = 112
            
        if 'board_size' in self.cfg['model']:
            self.BOARD_SIZE = self.cfg['model']['board_size']
        else:
            self.BOARD_SIZE = 8
        ...
        self.virtual_batch_size = self.cfg['model']['virtual_batch_size']
...
#         tf.config.experimental.set_memory_growth(gpus[self.cfg['gpu']], True)
    ...
    def init_net_v2(self):
        if self.cfg['model']['regularizer']:
            self.l2reg = tf.keras.regularizers.l2(l=0.5 * (0.0001))
        else:
            self.l2reg = None
#             print('No regularizer used')
        input_var = tf.keras.Input(shape=(self.INPUT_SIZE, self.BOARD_SIZE, self.BOARD_SIZE))
        x_planes = input_var #tf.keras.layers.Reshape([self.INPUT_SIZE, self.BOARD_SIZE, self.BOARD_SIZE])(input_var)
        ...
        if self.OUTPUT_SIZE == 1858:
            ...
            self.policy_loss_fn = 'mean_squared_error'
            self.policy_accuracy_fn = 'accuracy'
...
#         return tf.keras.layers.Dense(1858)(tf.keras.layers.Flatten()(inputs))
        ...
        if self.POLICY_HEAD == NetworkFormat.POLICY_CONVOLUTION:
            ...
            if self.OUTPUT_SIZE == 1858:
                conv_pol2 = tf.keras.layers.Conv2D(80, 3, use_bias=True, padding='same',
                                                   kernel_initializer='glorot_normal',kernel_regularizer=self.l2reg,
                                                   bias_regularizer=self.l2reg, data_format='channels_first')(conv_pol)
                h_fc1 = ApplyPolicyMap()(conv_pol2)
            else:
                pooled = tf.keras.layers.GlobalAveragePooling2D(data_format='channels_first')(conv_pol)
                squeezed = tf.keras.layers.Activation('relu')(tf.keras.layers.Dense(self.RESIDUAL_FILTERS // self.SE_ratio,
                                                                                    kernel_initializer='glorot_normal',
                                                                                    kernel_regularizer=self.l2reg)(pooled))
                h_fc1 = (tf.keras.layers.Dense(self.OUTPUT_SIZE, kernel_initializer='glorot_normal', kernel_regularizer=self.l2reg)(squeezed))
                h_fc1 = tf.keras.layers.Activation('sigmoid')(h_fc1)
            ...
            h_fc1 = tf.keras.layers.Dense(self.OUTPUT_SIZE, kernel_initializer='glorot_normal', kernel_regularizer=self.l2reg, bias_regularizer=self.l2reg)(h_conv_pol_flat)
        else:
            raise ValueError(
...
def get_tfp(path=None, filters=64, blocks=6, regularizer=True, gpu=0, virtual_batch_size=None, output_size=1858, input_size=123, board_size=8):
    config = {
        'gpu' : gpu,
        'model': {
    ...
    tfp = TFProcess(config, '', '')
    tfp.init_net_v2()
    if path:
        custom_objects={'policy_loss':tfp.policy_loss_fn,
                        'policy_accuracy':tfp.policy_accuracy_fn,
                        'ApplySqueezeExcitation':ApplySqueezeExcitation,
                        'ApplyPolicyMap':ApplyPolicyMap}
        tfp.model = tf.keras.models.load_model(path, custom_objects)
    return tfp